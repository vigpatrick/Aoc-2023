...
def get_hand_score(hand, bids, index)
  ...
  pair_count = 0
  triple_count = 0
  cards.each do |_card, qt|
    return { i: index, bid: bids[index], val: 5, hand: hand} if qt == 4
...
def order_hands(hands)
  priority_map = { "A" => 13, "K" => 12, "Q" => 11, "J" => 10, "T" => 9, "9" => 8, "8" => 7, "7" => 6, "6" => 5, "5" => 4, "4" => 3, "3" => 2, "2" => 1 }
...
File.readlines('7-1').each do |line|
  hands << line.split(' ')[0]

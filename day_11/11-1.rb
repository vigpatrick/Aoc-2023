...
galexies.each_with_index do |start, s|
  galexies.each_with_index do |goal, g|
    ...
    empty_rows.each do |row|
      distances["#{s}-#{g}"] += 999999 if row.between?(start[1], goal[1]) || row.between?(goal[1], start[1])
    end

    empty_cols.each do |col|
      distances["#{s}-#{g}"] += 999999 if col.between?(start[0], goal[0]) || col.between?(goal[0], start[0])
    end
  end
end
